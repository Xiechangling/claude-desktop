...
const DiffCard: React.FC<DiffCardProps> = ({ diff, onAccept, onReject }) => {
  ...
  // Theme detection optimized with useMemo - only compute when expanded
  // Avoids unnecessary DOM queries when diff viewer is not rendered
  const isDarkTheme = useMemo(() => {
    return isExpanded ? document.documentElement.classList.contains('dark') : false;
  }, [isExpanded]);
    ...
    <div className="border border-claude-border rounded-lg p-3 my-2 bg-claude-input">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-claude-text truncate max-w-md" title={diff.filePath}>{diff.filePath}</span>
          ...
          {diff.status === 'pending' && (
            <div className="flex gap-2 mt-2">
              <button
                ...
                aria-label={`Accept changes to ${diff.filePath}`}
              >
              ...
              <button
                ...
                aria-label={`Reject changes to ${diff.filePath}`}
              >
