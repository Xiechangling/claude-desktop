...
try {
    if (fs.existsSync(CUSTOM_SYSTEM_PROMPT_PATH)) {
        ...
        console.log(`[System Prompt] Loaded (${customSystemPrompt.length} chars)`);
    } else {
...
function initServer(mainWindow) {
    ...
    async function probeAnthropicWebSearch(p) {
        ...
        // Try Bearer auth first (used by most aggregators),
...
`;
    ...
    function resolveChatConfig(conv, env_token, env_base_url) {
    ...
    server.post('/api/conversations/:id/warm', (req, res) => {
        ...
        if (!conv) return res.status(404).json({ error: 'Not found' });
        const { env_token, env_base_url, user_profile } = req.body || {};
        const config = resolveChatConfig(conv, env_token, env_base_url);
    ...
    server.post('/api/chat', async (req, res) => {
        const { conversation_id, message, attachments, env_token, env_base_url, user_profile } = req.body;
